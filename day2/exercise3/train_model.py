"""
Script for training a ResNet and an Unet model for land segmentation.
...
# Train model
def train(model, train_loader, val_loader, loss_fn, optimizer, accuracy_fn, epochs, device, early_stopper):
    start = time.time()
    model.to(device)
    ...
    for epoch in range(epochs):
        ...
        for step, batch in enumerate(train_loader, 1):
            ...
            train_acc += accuracy_fn(output, y).item() * batch_size
            n_train += batch_size
        ...
        with torch.no_grad(), amp.autocast('cuda'):
            for batch in val_loader:
                ...
                val_acc += accuracy_fn(output, y).item() * batch_size
                n_val += batch_size
...
def main():
    ...
    output_folder = os.path.join(base_folder, os.environ.get('USER'), 'lumi-aif-fmi', 'day2', 'exercise3','model_training')
    ...
    # Create model
    # Choose here whether you want to train a ResNet or an UNET
